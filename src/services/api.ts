...
export async function getFeed(persona: Persona, location: string, signal?:AbortSignal): Promise<Feed> {
  const response = await fetch(endpoint(persona, location), {cache:"no-store", signal});
  if (!response.ok) throw new Error("Weather data is temporarily unavailable.");
  return response.json() as Promise<Feed>;
}

export async function getProfile(): Promise<UserProfile> {
  const response = await fetch("/api/users/demo-user", {cache:"no-store"});
  if (!response.ok) throw new Error("Could not load profile.");
  return response.json();
}

export async function updatePreferences(changes: Partial<UserProfile>): Promise<UserProfile> {
  const response = await fetch("/api/users/demo-user/preferences", {method:"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify(changes)});
  if (!response.ok) throw new Error("Could not save preferences.");
  return response.json();
}

export async function getLocations(): Promise<SavedLocation[]> {
  const response = await fetch("/api/users/demo-user/locations", {cache:"no-store"});
...
export async function addLocation(name:string, label="Other"): Promise<SavedLocation> {
  const response = await fetch("/api/users/demo-user/locations", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({name,label})});
...
export async function reportObservation(type: string, location: string, area:string, details:string):Promise<Observation> {
  const response = await fetch("/api/community/observations", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({type, location, area, details})});
...
export async function getObservations(location:string):Promise<Observation[]> {
  const response = await fetch(`/api/community/observations?location=${encodeURIComponent(location)}`, {cache:"no-store"});
  if(!response.ok) throw new Error("Reports could not be loaded");
  return response.json();
}

export async function markNotificationRead(id:string) {
  const response = await fetch(`/api/users/demo-user/notifications/${encodeURIComponent(id)}/read`, {method:"POST"});
  if(!response.ok) throw new Error("Could not update notification");
}

export async function getDemo(persona:Persona):Promise<Feed> {
  const response = await fetch(`/api/demo/scenarios/${persona}`, {method:"POST"});
